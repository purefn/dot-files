...
let
  ...
  toLayoutPath = p: "<path from=\"${p.from}\" to=\"${p.to}\" />\n";
...
<config version="2" xmlns="http://mediatomb.cc/config/2" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://mediatomb.cc/config/2 http://mediatomb.cc/config/2.xsd">
    ...
    <import hidden-files="no">
      ...
      <layout parent-path="${optionYesNo cfg.layout.parentPath}">
        ${concatMapStrings toLayoutPath cfg.layout.paths}
      </layout>
      <mappings>
...
in {
  ...
  options = {
    ...
    services.mediatomb = {
      ...
      layout.parentPath = mkOption {
        type = types.bool;
        default = false;
        description = ''
          Specifies if parent path is added to virtual layout.
        '';
      };

      layout.paths = mkOption {
        type = with types; listOf (submodule layoutPath);
        default = [];
        description = ''

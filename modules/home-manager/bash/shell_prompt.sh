...
function __promptline_ps1 {
  ...
  [ $is_prompt_empty -eq 1 ] && slice_prefix="$slice_empty_prefix"
  # section "z" slices
  __promptline_wrapper "$(git branch --no-color 2> /dev/null | sed -e "/^[^*]/d" -e "s/* \(.*\)/ \1/")" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; is_prompt_empty=0; }
...
function __promptline_right_prompt {
  ...
  # section "z" slices
  __promptline_wrapper "$(git branch --no-color 2> /dev/null | sed -e "/^[^*]/d" -e "s/* \(.*\)/ \1/")" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; }
